...
function run_sqg_ML(fac,dt_save,t_tot,Forc_fine)
    ...
    theta_fine = ncread("../data/SQG_noise2.nc", "theta")
    theta_coarse = coarsen_xy(theta_fine,fac)
    theta_coarsened = theta_coarse
    theta_ML = theta_coarse
    ...
    Forc_coarse = coarsen_xy(Forc_fine,fac)
    ...
    theta_coarsened_all = ncread(fname,"theta_coarsened")

    # Pre-allocating output arrays
    theta_coarse_all = zeros((Nx_coarse,Ny_coarse,nsave))
    theta_ML_all = zeros((Nx_coarse,Ny_coarse,nsave))

    # Building the CNN and Setting up with learnt parameters
    CNN_model = get_CNN_model()
    @load "../data/scalings.jld2" scalings

    # Running coarse model and coarse with P
    t = 0
    cnt_save = 1
    for it in 1:nit
        # Running SQG model
        t += dt
        theta_coarse = timestep_sqg(coarse_SQG,theta_coarse,Forc_coarse,dt)
        P_ML = scalings[3] .+ scalings[4]*reshape(CNN_model(add_singleton_dims( (theta_ML .- scalings[1])/scalings[2] )),(Nx_coarse,Nx_coarse))
        theta_ML = timestep_sqg_with_ML(coarse_SQG,theta_ML,Forc_coarse,P_ML,dt)

        # Saving
        if t%dt_save == 0
            theta_coarse_all[:,:,cnt_save] = theta_coarse
            theta_ML_all[:,:,cnt_save] = theta_ML
            cnt_save += 1
        end
    end

    # Timeseries of MSE
    Nt = size(theta_coarsened_all)[3]
    tt = (1:Nt)*dt
    MSE_theta_coarse = dropdims( (1/Nt^2)*mean( (theta_coarse_all - theta_coarsened_all).^2,dims=(1,2)),dims=(1,2))
    MSE_theta_ML = dropdims( (1/Nt^2)*mean( (theta_ML_all - theta_coarsened_all).^2,dims=(1,2)),dims=(1,2))
    return MSE_theta_coarse,MSE_theta_ML
end
